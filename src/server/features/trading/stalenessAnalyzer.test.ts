...
type StalenessResult = NonNullable<ReturnType<typeof computeStalenessScore>>;

function computeOrThrow(input: StalenessInput, now?: Date): StalenessResult {
	const result = computeStalenessScore(input, now);
	if (result === null) throw new Error("Expected non-null staleness result");
	return result;
}
...
describe("computeStalenessScore", () => {
	...
	describe("time held dimension (max 40pts)", () => {
		it("scores 0 when held less than 2 days", () => {
			const result = computeOrThrow(makeInput({ hoursHeld: 36 }));
			expect(result.timeHeldScore).toBe(0);
		});

		it("scores 0 at exactly 2 days", () => {
			const result = computeOrThrow(makeInput({ hoursHeld: 48 }));
			expect(result.timeHeldScore).toBe(0);
		});

		it("scores 40 at 3 days", () => {
			const result = computeOrThrow(makeInput({ hoursHeld: 72 }));
			expect(result.timeHeldScore).toBe(40);
		});

		it("scores 40 at 3+ days", () => {
			const result = computeOrThrow(makeInput({ hoursHeld: 120 }));
			expect(result.timeHeldScore).toBe(40);
		});

		it("scores linearly between 2 and 3 days (2.5 days = 20pts)", () => {
			const result = computeOrThrow(makeInput({ hoursHeld: 60 }));
			expect(result.timeHeldScore).toBe(20);
		});

		it("scores linearly at 2.25 days = 10pts", () => {
			const result = computeOrThrow(makeInput({ hoursHeld: 54 }));
	...
	describe("hoursHeld output", () => {
		it("reports correct hours held", () => {
			const result = computeOrThrow(makeInput({ hoursHeld: 50 }));
