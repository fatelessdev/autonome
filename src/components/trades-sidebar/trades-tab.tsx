...
function TradeItem({ trade, isLast }: { trade: Trade; isLast: boolean }) {
	...
	const timestampLabel = trade.timestamp ?? formatTimestamp(trade.closedAt);
	const holdingLabel =
		trade.holdingTime ?? computeHoldingLabel(trade.openedAt, trade.closedAt);
	const quantityLabel =
	...
	const entryNotional =
		trade.quantity != null && trade.entryPrice != null
			? trade.quantity * trade.entryPrice
			: null;
	const exitNotional =
		trade.quantity != null && trade.exitPrice != null
			? trade.quantity * trade.exitPrice
			: null;
	const entryNotionalLabel =
		entryNotional != null ? formatCurrency(entryNotional) : "--";
	const exitNotionalLabel =
		exitNotional != null ? formatCurrency(exitNotional) : "--";
	const pnlLabel = trade.netPnl != null ? formatCurrency(trade.netPnl) : "--";
	...
	return (
		<div>
			...
			{!isLast && <div className="mx-4 border-b" />}
		</div>
	);
}
