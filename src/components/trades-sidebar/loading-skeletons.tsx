...
export function TradesListSkeleton({ count = 3 }: { count?: number }) {
	return (
		<div className="space-y-4 p-4">
			{Array.from({ length: count }, (_, idx) => `trade-skeleton-${idx}`).map(
				(key) => (
					<div
						key={key}
...
export function PositionsListSkeleton({
	...
	return (
		<div className="space-y-4 p-4">
			{Array.from({ length: groups }, (_, idx) => `position-group-${idx}`).map(
				(groupKey) => (
					<div
						key={groupKey}
					...
					>
						...
						<div className="grid grid-cols-6 gap-x-2 border-b bg-muted/30 px-4 py-2">
							{Array.from(
								{ length: 6 },
								(_, idx) => `position-header-${groupKey}-${idx}`,
							).map((headerKey) => (
								<Skeleton key={headerKey} className="h-3 w-full" />
							))}
						</div>
						{Array.from(
							{ length: rows },
							(_, idx) => `position-row-${groupKey}-${idx}`,
						).map((rowKey) => (
							<div
								key={rowKey}
							...
							>
								{Array.from(
									{ length: 6 },
									(_, idx) => `position-cell-${rowKey}-${idx}`,
								).map((cellKey) => (
